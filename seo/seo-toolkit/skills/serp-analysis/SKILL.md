...
Persist the rendered report to `${CLAUDE_PLUGIN_DATA}/serp-analysis/<slug>-<YYYY-MM-DD>.md` using the `Write` tool, where `<slug>` is derived from the query (lowercase, non-alphanumerics → `-`).
